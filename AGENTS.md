...
- **Version bump touches 10 pinned literals, not 5.** `scripts/check.sh` requires one unified value across `lib/common.sh`, the three `payload.sh` files, `harvest.py`, and `scripts/package-release.sh` — *and* exact string matches in `README.md` (`version-3.3.0-green` badge), `CHANGELOG.md` (`^## [v3.3.0] - <date>$`, date included), `scripts/install-pager.sh`, `docs/architecture.dot`, plus the hard-coded `3.3.0` in `check.sh` itself. Bumping only the runtime declarations fails the gate.
